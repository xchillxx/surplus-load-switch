...
- **Dynamic wallbox surplus reservation** — a wallbox can also reserve a
  ...
  the ramp-up takes. Written as topping the configured house-load
  sensor's own reading up by only the shortfall (reservation minus real
  draw), not added a second time in full — that reading already
  includes the wallbox's real draw once it exists, verified against
  several hours of history across independent charging sessions (the
  reading reliably rises and falls in step with the wallbox's own draw
  on every ramp up/down, while its wallbox-off baseline stays well
  below any observed combined reading), so adding the full amount on
  top would double-count it. A brief gap in the car's own
  SOC/target/capacity/presence entities (a flaky cloud API, for
  instance) bridges to the
